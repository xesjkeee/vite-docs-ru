...
:::tip ПРИМЕЧАНИЕ
Существует конвенция выкладывать плагины Vite/Rollup в виде функции, которая возвращает плагин. Такая функция может получать настройки, которые позволяют пользователю управлять поведением плагина.
:::
